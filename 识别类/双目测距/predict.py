...
# 使用YOLO模型获取笔记本电脑的检测框
results = model(left_img, save=False, imgsz=640, conf=0.5, classes=[63])#只识别笔记本
...
def visualize_depth_map(depth_map):
    ...
    return cv2.applyColorMap(norm_depth.astype(np.uint8), cv2.COLORMAP_JET)
...
# 保存结果图像和距离结果
cv2.imwrite("深度图.png", depth_colored)  # 保存深度图
